...
class RuntimeTaskManager(ABC):
    ...
    def __str__(self):
        work_str = f"{self.len_qwork()}/{self.max_work_size}" if self.max_work_size > 0 else f"{self.len_qwork()}"
        return f"({len(self)}){self.fctn.__name__}[{work_str}|{self.len_qerrr()}|{self.len_qdone()}]"
